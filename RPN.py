...
brackets_set = {"(", ")"}
op_set = {"+", "-", "*", "/", "^"}
pref_op_set = {"~", "sqrt", "sin", "cos", "tg", "ctg", "ln","log", "log2", "lg", "arcsin", "arccos", "arctg", "arcctg"}
const_set = {"pi", "e"}
...
func_dict = {
    ...
    "log": lambda base, val: log(val, base)
}
    ...
    for char in string:
        if char == " ":
            if cur_str != "":
                ...
                cur_str = ""
        elif char in op_set or char in brackets_set or char == ",":
            if cur_str != "":
                str_arr.append(cur_str)
                cur_str = ""
            str_arr.append(char)
...
def validation(str_arr):
    if len(str_arr) == 0:
        return False
    for el in str_arr:
        if el != "," and el not in op_set and el not in pref_op_set and el != "x" \
...
def str_arr_to_rpn_arr(str_arr):
    ...
    priority_dict = {
        ...
        "/": 1,
        "^": 2,
        "(": -1,#чтобы не вытолкнули из стека
        #у всех префикстных функций приоритетет может быть >= 1, но логичнее 1, чтобы возведение в степень было более приоритетным
    }
